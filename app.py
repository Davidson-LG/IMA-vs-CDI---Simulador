...
# ── sys.path fix (Streamlit Cloud) ─────────────────────────────────────────────
# Adiciona todos os caminhos possíveis onde utils/ pode estar
def _fix_path():
    candidates = set()
    # 1. Diretório onde este arquivo está
    try:
    ...
    # 3. Caminhos típicos do Streamlit Cloud
    for src in ["/mount/src"]:
        if os.path.isdir(src):
            for sub in os.listdir(src):
                candidates.add(os.path.join(src, sub))
    # Insere apenas os que contêm utils/
    for p in candidates:
        utils_path = os.path.join(p, "utils")
        if os.path.isdir(utils_path) and p not in sys.path:
            sys.path.insert(0, p)
