...
def render():
    ...
    if data_fim <= data_inicio:
        st.error("⚠️ A data final deve ser posterior à data de início.")
        return

    # ── Configuração dos cenários ──────────────────────────────────────────────
    st.markdown("### ⚙️ Variação de Curva por Cenário")
    st.markdown(
        '<div class="info-box">Insira a variação esperada da taxa do IMA-B 5 em cada cenário. '
        'Valores negativos = fechamento (queda de taxa) = ganho de marcação. '
        'Positivos = abertura = perda de marcação.</div>',
    ...
    with col1:
        ...
        v1 = st.number_input(
            "Variação (p.p.)", min_value=-10.0, max_value=10.0,
            value=float(st.session_state.get("cenario1_variacao", -0.50)),
            step=0.05, format="%.2f", key="v1",
            help="Negativo = fechamento de curva (ex: -0.50)"
        )
    ...
    if not ipca_list:
        st.warning("⚠️ Nenhuma projeção de IPCA carregada. Acesse a aba **Parâmetros** e atualize o Focus.")
        return

    if not selic_list:
        st.warning("⚠️ Nenhuma projeção de Selic carregada. Acesse a aba **Parâmetros** e atualize o Focus.")
        return
    ...
    if vna_ini is None:
        st.warning("⚠️ VNA não encontrado para a data de início. Carregue o histórico VNA na aba **Histórico VNA**.")
        return

    # Projeta VNA
    with st.spinner("Calculando projeções VNA..."):
        df_vna_proj = project_vna_daily(data_inicio, data_fim, vna_ini, ipca_monthly, holidays)

    if df_vna_proj.empty:
        st.error("Erro ao calcular VNA projetado.")
        return

    vna_fim = float(df_vna_proj.iloc[-1]["VNA"])

    # Calcular os três cenários
    cenarios = [
        {"label": "Cenário 1", "subtitulo": "Fechamento de Curva", "variacao": v1, "cor": "#1E8449", "emoji": "🟢"},
        {"label": "Cenário 2", "subtitulo": "Estabilidade", "variacao": v2, "cor": "#D4AC0D", "emoji": "🟡"},
        {"label": "Cenário 3", "subtitulo": "Abertura de Curva", "variacao": v3, "cor": "#C0392B", "emoji": "🔴"},
    ]
    ...
    for c in cenarios:
        res_imab = calcular_retorno_imab5(
            ...
            vna_ini, vna_fim, c["variacao"], holidays
        )
    ...
    dur_anos = duration_du / 252.0

    # Info bar
    st.markdown(f"""
    <div class="info-box" style="display:flex; gap:40px; flex-wrap:wrap;">
        <span>📅 <b>Dias Úteis:</b> {du_total}</span>
        <span>📉 <b>IPCA Acumulado:</b> {ipca_acum:.4f}%</span>
        <span>⏱ <b>Duration:</b> {dur_anos:.2f} anos</span>
        <span>📌 <b>Cenário:</b> {st.session_state.get('cenario_ativo_ipca','base').capitalize()}</span>
        <span>🏦 <b>Selic:</b> {st.session_state.get('cenario_ativo_selic','base').capitalize()}</span>
    </div>
    """, unsafe_allow_html=True)
    ...
    cols = st.columns(3)
    for i, (col, res) in enumerate(zip(cols, resultados)):
        with col:
            _render_cenario_card(res, i)

    # ── Gráfico comparativo ────────────────────────────────────────────────────
    st.markdown("---")
    st.markdown("### 📈 Comparativo Visual")
    _render_grafico_comparativo(resultados)

    # ── Tabela detalhada ───────────────────────────────────────────────────────
    with st.expander("📋 Tabela Detalhada por Cenário", expanded=True):
        _render_tabela_detalhada(resultados)


def _render_cenario_card(res: dict, idx: int):
    imab = res["imab"]
    cdi = res["cdi"]
    ...
    cor_dif = res["cor"] if dif >= 0 else "#C0392B"
    sinal = "+" if dif >= 0 else ""
    ...
    <div style="background:white; border-radius:14px; padding:20px;
        ...
        <div style="font-size:0.85rem; color:#7F8C8D; margin-bottom:14px;">
            {res['subtitulo']} | Δtaxa: {res['variacao']:+.2f} p.p.
        </div>
        ...
        <div style="background:#F8F9FA; border-radius:8px; padding:10px; text-align:center;">
            ...
            <div style="font-size:1.3rem; font-weight:700; color:{cor_dif};">
                {sinal}{dif:.4f} p.p.
            </div>
            <div style="font-size:0.75rem; color:{cor_dif}; font-weight:600;">
                ✦ {vencedor} mais vantajoso
            </div>
        </div>
        ...
        <div style="margin-top:12px; font-size:0.78rem; color:#7F8C8D; border-top:1px solid #ECF0F1; padding-top:8px;">
            ...
            <div>• IPCA: {imab['ipca_periodo']*100:.4f}%</div>
        </div>
    </div>
    """, unsafe_allow_html=True)


def _render_grafico_comparativo(resultados: list):
    cenarios_labels = [r["label"] for r in resultados]
    ...
    fig.add_trace(go.Bar(
        ...
        text=[f"{v:.4f}%" for v in ret_imab],
        textposition="outside",
        textfont=dict(size=12, color="#1B4F72"),
    ))
    ...
    fig.update_layout(
        barmode="group",
        title="Retorno Esperado por Cenário (%)",
        yaxis_title="Retorno (%)",
        height=420,
        plot_bgcolor="white",
        paper_bgcolor="white",
    ...
    # Gráfico de componentes IMA-B 5
    ...
    fig2.add_trace(go.Bar(
        name="Carrego (real + IPCA)",
        x=cenarios_labels, y=carrego,
        marker_color="#2E86C1",
        text=[f"{v:.4f}%" for v in carrego],
        textposition="inside",
    ))
    fig2.add_trace(go.Bar(
        name="Marcação a Mercado",
        x=cenarios_labels, y=mtm,
        marker_color=["#1E8449" if v >= 0 else "#C0392B" for v in mtm],
        text=[f"{v:+.4f}%" for v in mtm],
        textposition="inside",
    ))
...
def _render_tabela_detalhada(resultados: list):
    rows = []
    for res in resultados:
        imab = res["imab"]
        cdi = res["cdi"]
        rows.append({
            "Cenário": f"{res['emoji']} {res['label']} — {res['subtitulo']}",
            "Δ Taxa (p.p.)": f"{res['variacao']:+.2f}",
            "IPCA (%)": f"{imab['ipca_periodo']*100:.4f}",
