    ...
    with subtab1:
        ...
        if st.session_state.get("selic_base"):
            ...
            if st.button("💾 Salvar Datas e Taxas COPOM", key="save_selic_base_manual"):
                ...
                # Propaga para cenários otimista e alternativo
                if not st.session_state.get("selic_otimista"):
                    st.session_state["selic_otimista"] = rows_updated.copy()
                if not st.session_state.get("selic_alternativo"):
                    st.session_state["selic_alternativo"] = rows_updated.copy()
                st.success("✅ Datas e taxas COPOM atualizadas! Todas as abas foram atualizadas.")
                st.rerun()
