...
def _render_selic_tab():
    ...
    if df_selic_focus.empty:
        ...
        # get_focus_selic_copom já retorna com colunas: Reuniao, data_reuniao, taxa_aa
        rows = []
    ...
    with subtab1:
        ...
        if st.session_state.get("selic_base"):
            ...
            df_b_show = df_b[["reuniao_label", "taxa_aa"]].copy()
            df_b_show.columns = ["Reunião COPOM", "Selic (%a.a.)"]
            df_b_show["Data Reunião"] = pd.to_datetime(df_b["data_reuniao"]).dt.strftime("%d/%m/%Y")
            df_b_show = df_b_show[["Reunião COPOM", "Data Reunião", "Selic (%a.a.)"]]
