...
def _focus_ipca_fallback() -> pd.DataFrame:
    """
    Fallback com valores do Focus de 17/04/2026 (último publicado conforme prints).
    Atualizar manualmente se API ficar indisponível por período longo.
    """
    valores_2026 = {
    ...
    for _ in range(28):
        v = valores_2026.get(ref.month, 0.30) if ref.year == 2026 else 0.30
        rows.append({
            "DataReferencia": pd.Timestamp(ref),
            "Mediana": v,
            "MesLabel": ref.strftime("%m/%Y"),
            "FocusData": "17/04/2026 (fallback)",
        })
