...
def get_focus_ipca_mensal() -> pd.DataFrame:
    ...
    # 1. Busca dados mensais do Focus (já retorna a data de publicação)
    mensal_map, data_pub = _fetch_ipca_mensal()   # {date: mediana%}, str

    # 2. Busca dados anuais do Focus para preencher lacunas
    anual_map = _fetch_ipca_anual()    # {ano: mediana%}

    # Salva data de publicação no session_state para exibição
    if data_pub:
        try:
            # Converte "2026-04-17" para "17/04/2026"
            dt = datetime.strptime(data_pub[:10], "%Y-%m-%d")
            st.session_state["focus_data_publicacao"] = dt.strftime("%d/%m/%Y")
        except Exception:
            st.session_state["focus_data_publicacao"] = data_pub
...
def _fetch_ipca_mensal() -> tuple[dict, str]:
    """Busca medianas mensais de IPCA. Retorna ({date: mediana%}, data_publicacao)."""
    result, data_pub = _ipca_mensal_via_bcb()
    if result:
        return result, data_pub
    return _ipca_mensal_via_requests()


def _ipca_mensal_via_bcb() -> tuple[dict, str]:
    try:
        ...
        if df.empty:
            return {}, ultima
        return _parse_mensal_list(df.to_dict("records")), ultima
    except Exception:
        return {}, ""


def _ipca_mensal_via_requests() -> tuple[dict, str]:
    ...
    return _parse_mensal_list(_get(url)), ultima
...
def get_focus_data_publicacao() -> str:
    """Retorna data da última publicação Focus (já formatada como dd/mm/yyyy)."""
    # Tenta ler do session_state (populado ao buscar IPCA)
    cached = st.session_state.get("focus_data_publicacao", "")
    if cached and cached != "N/D":
        return cached
