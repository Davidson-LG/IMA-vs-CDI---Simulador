...
def calcular_retorno_cdi(
    ...
    """
    CDI mensal — fórmula exata da planilha CDI MÊS A MÊS:

    Sem COPOM no mês:
        retorno = (1 + taxa)^(DU/252) - 1

    Com COPOM no mês (nova taxa vale A PARTIR do dia do COPOM, inclusive):
        retorno = (1+t1)^(DU1/252) × (1+t2)^(DU2/252) - 1
        onde DU1 inclui o dia do COPOM, DU2 são os dias após o COPOM.

    Múltiplas reuniões são encadeadas da mesma forma.
    """
    ...
    # Taxa vigente no início do período
    taxa_atual = reunioes[0]["taxa_aa"] if reunioes else 14.75
    for r in reunioes:
        if r["data_reuniao"] <= data_inicio:
            taxa_atual = r["taxa_aa"]

    # Reuniões com mudança de taxa que ocorrem DENTRO do período
    # (estritamente após data_inicio e até data_fim)
    reunioes_no_periodo = [
        r for r in reunioes
        if data_inicio < r["data_reuniao"] <= data_fim
        and r["taxa_aa"] != taxa_atual
    ]
    # Remove duplicatas mantendo a última por data
    seen_dates = {}
    for r in reunioes_no_periodo:
        seen_dates[r["data_reuniao"]] = r
    reunioes_no_periodo = sorted(seen_dates.values(), key=lambda x: x["data_reuniao"])

    # Reconstrói reunioes_no_periodo incluindo mudanças sequenciais
    # (pode haver casos onde taxa muda várias vezes no período)
    reunioes_validas = []
    t = taxa_atual
    for r in sorted(reunioes, key=lambda x: x["data_reuniao"]):
        if r["data_reuniao"] <= data_inicio:
            t = r["taxa_aa"]
        elif r["data_reuniao"] <= data_fim and r["taxa_aa"] != t:
            reunioes_validas.append(r)
            t = r["taxa_aa"]

    if not reunioes_validas:
        # Sem mudança de taxa: fórmula simples
        du = len(all_days)
        retorno = (1 + taxa_atual / 100.0) ** (du / 252.0) - 1.0
        return {
            "retorno_cdi": retorno,
            "du": du,
            "taxa_media": taxa_atual,
            "indice_final": 1.0 + retorno,
        }

    # Com mudanças: divide em segmentos por data de reunião
    # Segmento: todos os DU até E INCLUINDO o dia da reunião → taxa_antes
    #           todos os DU APÓS a reunião → taxa_depois
    indice = 1.0
    soma_ponderada = 0.0
    dias_restantes = list(all_days)
    taxa_seg = taxa_atual

    for r in reunioes_validas:
        data_cop = r["data_reuniao"]
        # Dias deste segmento: até e incluindo o dia do COPOM
        dias_seg = [d for d in dias_restantes if d <= data_cop]
        if dias_seg:
            du_seg = len(dias_seg)
            fator = (1 + taxa_seg / 100.0) ** (du_seg / 252.0)
            indice *= fator
            soma_ponderada += taxa_seg * du_seg
        dias_restantes = [d for d in dias_restantes if d > data_cop]
        taxa_seg = r["taxa_aa"]

    # Último segmento (após última reunião)
    if dias_restantes:
        du_seg = len(dias_restantes)
        fator = (1 + taxa_seg / 100.0) ** (du_seg / 252.0)
        indice *= fator
        soma_ponderada += taxa_seg * du_seg
    ...
    return {
        ...
        "du": du_total,
        "taxa_media": soma_ponderada / du_total if du_total > 0 else 0.0,
        "indice_final": indice,
    }
